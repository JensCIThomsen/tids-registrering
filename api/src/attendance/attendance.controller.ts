...
export class AttendanceController {
  ...
  async dbCheck() {
    ...
    const attendanceCount = await this.prisma.$queryRaw<{ count: number }[]>`
  ...
  async todayJwt(@Req() req: Request) {
    ...
    if (jwtUser.role !== 'COMPANY_ADMIN') {
      throw new ForbiddenException('Kun COMPANY_ADMIN kan se today-overblik');
    }
    ...
    const gaaet = rows.filter((r) => r.type === AttendanceType.GAAET).length;
  ...
  async today(@Param('companyId') companyId: string, @Req() req: Request) {
    ...
    if (jwtUser.role !== 'SUPERADMIN') {
      ...
      if (jwtUser.role !== 'COMPANY_ADMIN' || jwtUser.companyId !== companyId) {
        throw new ForbiddenException('Du har ikke adgang til denne virksomhed');
      }
    }
  ...
  async gaaet(@Req() req: Request) {
    ...
    if (
      ...
      return {
        ok: false,
        error:
          'Du kan ikke gået, fordi du ikke er mødt (sidste status er ikke MOEDT/PAUSE_END)',
        last,
      };
    }
  ...
  async pauseStart(@Req() req: Request) {
    ...
    if (last.type === AttendanceType.GAAET) {
      return {
        ok: false,
        error:
          'Du kan ikke starte pause, fordi du ikke er mødt (sidste status er GAAET)',
        last,
      };
    }
  ...
  async pauseEnd(@Req() req: Request) {
    ...
    if (last.type === AttendanceType.GAAET) {
      return {
        ok: false,
        error:
          'Du kan ikke slutte pause, fordi du ikke er mødt (sidste status er GAAET)',
        last,
      };
    }
